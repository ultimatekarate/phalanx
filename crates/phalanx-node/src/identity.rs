...
impl PhalanxIdentity {
    pub fn generate() -> Result<(Self, String), IdentityError> {
        ...
        let mnemonic = Mnemonic::from_entropy(&entropy).map_err(|e| IdentityError::EntropyError(e.to_string()))?;
        let phrase = mnemonic.to_string();
        let seed = mnemonic.to_seed("");
        
        let secret_bytes: [u8; 32] = seed[0..32].try_into().map_err(|_| IdentityError::CryptoError("Seed fail".into()))?;
        let signing_key = SigningKey::from_bytes(&secret_bytes);

        Ok((PhalanxIdentity { version: IDENTITY_VERSION, did: Did::placeholder(), keypair: signing_key }, phrase))
    }

    pub fn restore(phrase: &str) -> Result<Self, IdentityError> {
        let mnemonic = Mnemonic::parse_in(Language::English, phrase).map_err(|e| IdentityError::MnemonicError(e.to_string()))?;
        let seed = mnemonic.to_seed("");
        let secret_bytes: [u8; 32] = seed[0..32].try_into().map_err(|_| IdentityError::CryptoError("Seed fail".into()))?;
        
        Ok(PhalanxIdentity { version: IDENTITY_VERSION, did: Did::placeholder(), keypair: SigningKey::from_bytes(&secret_bytes) })
    }

    pub fn save_to_disk<P: AsRef<Path>>(&self, path: P) -> Result<(), IdentityError> {
        let bytes = postcard::to_stdvec(self).map_err(|e| IdentityError::SerializationError(e.to_string()))?;
