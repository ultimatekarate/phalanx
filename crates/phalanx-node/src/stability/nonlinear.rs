...
impl NonlinearSystem {
    ...
    /// Decrement the reconnection burst timer by `dt` seconds.
    /// Clears the burst multiplier when the timer expires.
    pub(super) fn tick_burst(&mut self, dt: f64) {
        if self.burst_remaining > 0.0 {
            self.burst_remaining = (self.burst_remaining - dt).max(0.0);
            if self.burst_remaining <= 0.0 {
