...
| Off-grid mesh | Works over a local IP link | Off-grid = the same mesh on a shared WiFi/hotspot network with no internet upstream; one manual networking step. A BLE seam was deliberately excised in July 2026 ([network.md §6](network.md#6-off-grid-operation)). |
...
| 8. Corroborate | `stronghold corroborate -c <hex> --grants g1.bin --recordings r1 r2` — **minimum two** recording ids (`num_args = 2..`, `bin/stronghold.rs:63-64`), overlapping by ≥ `min_overlap_ms` | `Corroboration proof produced:` with proof hash, attestation/divergence counts, producer DID |
