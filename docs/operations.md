...
| Background recording on Android | **UNTESTED — likely stops when backgrounded** | The manifest declares `FOREGROUND_SERVICE`/`FOREGROUND_SERVICE_CAMERA`/`FOREGROUND_SERVICE_MICROPHONE` permissions but contains **no `<service>` element**, so no foreground service exists to keep capture alive (`AndroidManifest.xml:23-80`). The app does forward lifecycle to Rust (backgrounded → Dormant, `flutter_app/lib/main.dart:270-286`), which is the designed homeostatic response — but "record while screen-off" is not a capability anyone has verified. |
| BLE / WiFi-Direct off-grid mesh | Not implemented | The Rust seam is complete and tested; no radio code exists on the Flutter side ([network.md §6](network.md#6-local-mesh-ble--wifi-direct)). |
...
(`flutter_app/android/app/src/main/AndroidManifest.xml:48-67`). The `phalanx://` custom scheme is registered only in
...
| Background capture | No `<service>` in the manifest despite the three foreground-service permissions (§1) | Declare and implement a camera/microphone foreground service; then test the backgrounded-recording claim on hardware. |
