...
| BLE / WiFi Direct local mesh | **Excised (July 2026, deliberate).** The full seam — FFI surface, BLE mutual auth, `LocalMeshPort`, proximity witnesses, the `Evidence::Proximity` variant, and the LocalMesh admission lanes — was removed after the owner concluded BLE carries no evidence (video exceeds its bandwidth) and its proximity witnesses were never load-bearing ("supporting, not required"). Off-grid operation is the existing IP mesh over a local WiFi/hotspot link ([network.md §6](network.md#6-off-grid-operation)). The removal re-indexed the postcard `Evidence` enum (pre-release, no field data). History lives in git. | `docs/network.md` §6 |
...
| Mesh "over QUIC and TCP", off-grid via a local WiFi/hotspot link (README § Capabilities) | QUIC **code-anchored** | QUIC: `crates/phalanx-transport/src/builder.rs:25-48` | The BLE/WiFi-Direct seam was excised in July 2026 (§2); off-grid = the same IP mesh on a local link, one manual networking step ([network.md §6](network.md#6-off-grid-operation)) |
...
| Off-grid local link | The mesh already runs over any shared IP link; what's missing is *forming* one with no internet: Android multicast permissions + MulticastLock, and a guided hotspot/WiFi-Direct join flow | Field-tested phone-to-phone capture-to-mesh over a local WiFi/hotspot link with the internet cut | [network.md §6](network.md#6-off-grid-operation) |
...
flowchart LR
    ...
    subgraph M24["Months 12–24 — the full story"]
        F[Off-grid local-link flow<br/>multicast + hotspot join] --> G[Audit remediation<br/>field pilot with partner org]
    end
    M6 --> M12 --> M24
```
