...
- **The phone node** — not a binary but a C-ABI library: `crates/phalanx-ffi` builds `["cdylib", "staticlib", "rlib"]` (`crates/phalanx-ffi/Cargo.toml:15`) for Android/iOS, bound by the Flutter app in `flutter_app/`. Real production capture enters here: Flutter's camera plugin pushes raw NV21/NV12 YUV planes into `phalanx_push_video_frame` (`crates/phalanx-ffi/src/capture.rs:293`). The mesh runs over IP (QUIC and TCP, `crates/phalanx-node/src/config.rs:226-231`); off-grid operation is the same mesh over a local WiFi/hotspot link ([network.md §6](network.md#6-off-grid-operation)).
...
| **Evidence (enum)** | The closed set of forensic payload kinds: exactly five variants — `Video(VideoShard)`, `Audio(AudioShard)`, `Gap(ForensicGap)`, `Handover(HandoverProof)`, `ManifestEntry(ManifestEntry)`. | `crates/phalanx-proto/src/evidence/envelope.rs:79` |
...
8. **Mobile is ephemeral; the Stronghold persists.** Rosters, watch sets, and replay filters are RAM-only on the seizable device class and durable on the operator-safe one ([threat-model.md §17](threat-model.md), `crates/phalanx-node/src/trust.rs:186`, `crates/phalanx-node/src/vitals/canary.rs:9`). *Rationale:* persistence is routed by physical threat profile. Do not "fix" mobile ephemerality — it is the defense, not a gap.
